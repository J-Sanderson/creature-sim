...
describe('showStatusWrapper', () => {
    test('adds status wrapper to world elements', () => {
        ...
        let world = worldBuilder({entities: { items: [] }});
    ...
    test('status wrapper has status-wrapper class', () => {
        ...
        expect(Array.from(world.elements.statusWrapper.classList)).toContain('status-wrapper');
    });
    ...
    test('status wrapper is child of world root element', () => {
        ...
        expect(world.elements.statusWrapper.parentElement).toBe(world.elements.root);
    });
});